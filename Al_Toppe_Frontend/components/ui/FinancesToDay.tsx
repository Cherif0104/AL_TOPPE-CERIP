...
export default function FinancesScreen() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const results: TransactionItemData[] = (response?.results ?? []);
      setTransactions(results);
      const today = new Date().toISOString().split('T')[0];
      setTransactions(results.filter((t) => new Date(t.date).toISOString().split('T')[0] === today));
      ...
      setAllCategories(categoriesList); // Stocker toutes les catégories
      
      // Filtrer selon le type de transaction initial
      const filteredCats = categoriesList.filter((cat: CategoryItem) => {
        ...
        >
          ...
          <View style={styles.balanceCard}>
            <View style={styles.balanceHeader}>
              ...
              <Text style={styles.balanceLabel}>Solde Net Aujourd'hui</Text>
           <TouchableOpacity style={styles.addButtonHero} onPress={() => setShowAddModal(true)}>
              <Plus size={24} color="#FFFFFF" />
            </TouchableOpacity>
            </View>
